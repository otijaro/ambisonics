...
export function DemoClient() {
  ...
  const audioResultsRef = useRef<HTMLDivElement>(null)

  const handleProcessDemo = async () => {
    if (!file) return
    if (status === 'processing') return // Evitar múltiples peticiones

    setStatus('processing')
    setError(null)

    try {
      const direccion = Number.isFinite(Number(demoControls.direccion)) ? Number(demoControls.direccion) : 0
      const altura = Number.isFinite(Number(demoControls.altura)) ? Number(demoControls.altura) : 0
      const apertura = Number.isFinite(Number(demoControls.apertura)) ? Number(demoControls.apertura) : 0
      const movimiento = Number.isFinite(Number(demoControls.movimiento)) ? Number(demoControls.movimiento) : 0

      const res = await runDemo(file, { direccion, altura, apertura, movimiento, original_mode: originalMode })

      // Pausar y resetear solo los audios dentro del área de resultados del Demo
      if (audioResultsRef.current) {
        audioResultsRef.current.querySelectorAll('audio').forEach((audio) => {
          audio.pause()
          audio.currentTime = 0
        })
      }
      ...
      <div className="grid gap-8 lg:grid-cols-2 items-start">
        ...
        <div className="rounded-3xl glass p-6 sm:p-8">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1.5 text-xs text-muted-foreground">
                  Hace que el sonido se desplace automáticamente alrededor del oyente. <br />
                  <span className="text-primary/80 font-medium mt-1 inline-block">Este control modifica únicamente la salida Binaural 3D Perceptual.</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="mt-8 flex h-12 w-full items-center justify-center rounded-xl bg-primary px-10 text-base font-bold text-primary-foreground hover:bg-primary/90 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:pointer-events-none"
          >
            {status === 'processing' ? (
              <>
              ...
              <>
                <RefreshCw className="mr-2 h-5 w-5" /> Procesar Demo
              </>
            )}
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl glass p-6 sm:p-8">
            ...
            {status !== 'done' ? (
              ...
              <AnimatePresence>
                <motion.div ref={audioResultsRef} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 space-y-4">
                  <AudioResult
