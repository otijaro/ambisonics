...
def convert_audio(input_path: str, output_dir: str, mode: str, hrtf, pos):
    ...
    if audio_duration < 60.0:
        ...
        f_qh = sf.SoundFile(quad_h_wav, 'w', sr, channels=4, subtype='FLOAT')
        f_qv = sf.SoundFile(quad_v_wav, 'w', sr, channels=4, subtype='FLOAT')
        f_qh3d = sf.SoundFile(quad_h_3d_wav, 'w', sr, channels=4, subtype='FLOAT')
        f_qv3d = sf.SoundFile(quad_v_3d_wav, 'w', sr, channels=4, subtype='FLOAT')
        ...
        for fpath in [binaural_wav, quad_h_wav, quad_v_wav, quad_h_3d_wav, quad_v_3d_wav]:
            arr, _ = sf.read(fpath)
    ...
    return audio_duration, time_total
...
def process_demo(input_path: str, output_dir: str, direccion: float, altura: float, apertura: float, movimiento: float, hrtf, pos):
    ...
    start_bin = time.time()
    if hrtf is not None and pos is not None:
        binaural, _, _ = foa_to_binaural(W, X, Y, Z, sr, hrtf, pos, flip_az=True, virtual_speakers=DEMO_VIRTUAL_SPEAKERS)
    else:
    ...
    start_3d = time.time()
    # 4. Binaural 3D con FOA modificado
    W_3d, X_3d, Y_3d, Z_3d = foa_motion_demo(
        W, X, Y, Z, sr, 
        rate_az=0.12, rate_el=0.10, max_el_deg=60.0, z_preview_gain=3.5
    )
    if hrtf is not None and pos is not None:
        binaural_3d, _, _ = foa_to_binaural(W_3d, X_3d, Y_3d, Z_3d, sr, hrtf, pos, flip_az=True, virtual_speakers=DEMO_VIRTUAL_SPEAKERS)
    else:
